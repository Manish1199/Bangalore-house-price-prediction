...
with open('C:\\Projects\\Benglore House Predictions\\model.pkl', 'rb') as file:
    model = pickle.load(file)

  # Truncated for brevity

locations = ['Devarachikkanahalli', '1st Block Jayanagar',
...
def predict():
    try:
        ...
        squarefoot = float(request.form['totalSqft'])
        bhk = int(request.form['bhk'])
        bathroom = int(request.form['bathroom'])
        location = request.form['location']

        # Create an input array with the expected number of features
        input_features = np.zeros(len(locations) + 4)  # Adjust the length based on your model
        input_features[0] = balcony
        input_features[1] = squarefoot
        input_features[2] = bhk
        input_features[3] = bathroom

        if location in locations:
            location_index = locations.index(location)
            input_features[4 + location_index] = 1
        ...
        return jsonify({'error': f"KeyError: {e}, please ensure all form fields are correctly filled."}), 400
